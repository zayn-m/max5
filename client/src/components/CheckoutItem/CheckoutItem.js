...
const checkoutItem = ({ cartItem, clearItem, addItem, removeItem, clicked }) => {
	...
	return (
		...
		<div className="row">
			<div className="col-2">
				<img src={imageUrl} className="w-75" alt={name} />
			</div>
			<div className="col-3">
				<strong>{name}</strong>
			</div>
			<div className="col-2">
				<span className="checkout-item__quantity--arrow" onClick={() => removeItem(cartItem)}>
					&#10094;
				</span>
				<span>{quantity}</span>
			</div>
			...
			<div className="col-2">
				<span className="checkout-item__remove-button" onClick={() => clearItem(cartItem)}>
					&#10005;
				</span>
			</div>
		</div>
